...
global $wpdb;
    ...
    $view = '<input id="post_media" type="file" name="post_media" value="' . get_post_meta($post->ID, 'post_media', true) . '" size="25" />';
...
function javiervd_filter_mime_types($mimes)
{
  return array('csv' => 'text/csv');
            ...
            $upload = wp_handle_upload($_FILES[$data_key], $overrides);
 
            if(isset($upload['error'])) {
                wp_die('There was an error uploading your file. Please try again.'.$upload['error']);
            } else {
...
add_action('save_post', 'save_custom_meta_data');
	...
	function list_the_codes() {
		...
		$thead .= '<tr bgcolor="#99CCFF"><td align="center" colspan="6"><a href="http://logitech/wp-admin/tools.php?page=export&rmcsv='.$table_name.'" id="csvDownload">Download The complete .CSV file here</a></td></tr>';
	...
	function codex_add_help_text( $contextual_help, $screen_id, $screen ) { 
		  ...
		  if ( 'page' == $screen->id ) {
			$contextual_help =
			  '<p>' . __('Things to remember when adding or editing a promo:') . '</p>';
		  } elseif ( 'edit-page' == $screen->id ) {
			$contextual_help = 
			  '<p>' . __('This is the help screen displaying the table of promos.') . '</p>' ;
		  }
		  if ( 'promo' == $screen->id ) {
			$contextual_help =
			  '<p>' . __('Things to remember when adding or editing a promo:') . '</p>';
